...
int main(int argc, char* argv[]) {
	...
	while(1) {			
        ...
        int rv = waitpid(-1,&stat,WNOHANG);  // -1 : meaning wait for any child process remaining or processes which became a zombie.
        if(rv > 0)
        {
       ...
       //do whatever you want with the commands, here we just print them
		...
		else if(fc == 0) // child's code
		{
			...
			{
                setpgid(fc,to_protect);
				fl = 0;
				tokens[numberoftokens-1] = '\0';
			}
			char temp_dir[] = "/bin/";
			strcat(temp_dir,tokens[0]);
			execv(temp_dir,tokens);
